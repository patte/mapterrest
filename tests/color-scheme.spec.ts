...
test('the browser colour scheme picks the basemap', async ({ browser }) => {
  for (const [colorScheme, expected] of [
    ...
    const picked = await themed.evaluate(() => (document.getElementById('basemap') as any).value);
...
test('an explicit basemap beats the scheme', async ({ browser }) => {
  ...
  await check(
    await chosen.evaluate(() => (document.getElementById('basemap') as any).value === 'carto-light'),
